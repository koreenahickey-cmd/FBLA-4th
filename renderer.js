  ...
  filtered.forEach(biz => {
    ...
    card.innerHTML = `
      <div class="card-header">
        ...
        <div class="rating-chip">⭐ ${biz.averageRating.toFixed(1)}</div>
      </div>
...
function continueAsGuest() {
  currentUser = { id: 'guest', name: 'Guest', email: 'guest', role: 'guest' };
  enterApp();
}
...
function openDetail(businessId) {
  ...
  const reviewsHTML = biz.reviews && biz.reviews.length
    ? biz.reviews.map(r => `<div class="review"><div><strong>${r.userName}</strong> • ${new Date(r.date).toLocaleDateString()}</div><div>⭐ ${r.rating}</div><p>${r.comment}</p></div>`).join('')
  ...
  body.innerHTML = `
    <div class="detail-header">
      ...
      <div class="rating-large">⭐ ${biz.averageRating.toFixed(1)}</div>
    </div>
